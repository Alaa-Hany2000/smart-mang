...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">{{trans("main.Units")}}</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="#">{{trans("main.Units")}}</a></li>
                            <li class="breadcrumb-item active">{{trans("main.Units")}}</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">{{trans("main.Blogs Units DataTable")}}</h3>
                            </div>
                            <div class="card-header" style="border-bottom: none">
                                <button class="btn btn-primary" data-toggle="modal" data-target="#createUnit">{{trans("main.Add Unit")}}</button>
                            </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">{{trans("main.Edit Unit")}} <span id="Unit_name"></span></h4>
                ...
                <form action="#" method="post" role="form">
                    ...
                    <div class="modal-footer justify-content-between">
                        ...
                        <button type="button" class="btn btn-primary" id="save_Unit">{{trans("main.Save changes")}}</button>
                    </div>
                </form>
            </div>
            <!-- /.modal-content -->
    ...
    <!-- /.modal -->
    <!-- /.modal -->
    <div class="modal fade" id="createUnit">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">{{trans("main.Add Unit")}}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <form action="#" class="form-horizontal" method="post" role="form">
                    ...
                    <div class="modal-footer justify-content-between">
                        <button type="button" class="btn btn-default" data-dismiss="modal">{{trans("main.Close")}}</button>
                        <button type="button" class="btn btn-primary" id="addUnit">{{trans("main.Save")}}</button>
                    </div>
                </form>
            </div>
            <!-- /.modal-content -->
        ...
        $(document).ready(() => {
            ...
            $("body").on("click", ".editUnit", function (){
                ...
                if(image == '')
                {
                    ...
                    $("#img_view").attr("src", "{{asset('upload/blog_Unit')}}" + "/" + image)
            ...
            // =========== Update Unit ========
            $('body').on('click', '#save_Unit', function () {
                let Unit_title = $("#title").val();
                let id = $("#id").val();
                let lang_id = $('#info').val();
                // console.log($('#published').val())
                // return ;
                let formData = new FormData();
                formData.append('title', Unit_title);
                ...
                if (img_url){
                    ...
                    $.ajax({
                        ...
                        url: "{{ url('admin/adminUnits/updatee') }}",
                        type: "get",
                        data: formData,
                        contentType : false,
                        processData : false,
                    ...
                    $.ajax({
                        ...
                        url: "{{ asset('admin/adminUnits/updatee') }}",
                        method: "post",
                        data: formData,
                        contentType : false,
                        processData : false,
            ...
            // =========== Create new Unit ========
            $("#addUnit").on('click', () => {
                let Unit_title = $("#create_title").val();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            ...
                            url: "{{ asset('admin/adminUnits/delete') }}" + '?id=' + id,
                            method: "delete",
                            type: 'DELETE',
                            statusCode: {
                                200: (response) => {
            ...
            $('body').on('change', '.onoffswitch-checkbox', function () {
                ...
                $.ajax({
                    ...
                    url: "{{  url('/admin/adminUnits/update') }}",
                    method: "get",
                    data: {
